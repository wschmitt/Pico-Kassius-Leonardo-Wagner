...
void print_inst_tac(FILE* out, struct tac i) {
	...
	else if (strcmp(i.op, "GOTO") == 0) { //gera instrucao pro if
		fprintf(out, "%s %s\n", i.res, i.arg2);
	}
	else if (strcmp(i.op, "IF") == 0) { //gera instrucao pro goto
		fprintf(out, "%s %s GOTO %s\n", i.res, i.arg1, i.arg2);
	}
	else if (strcmp(i.op, "LABEL") == 0) {
		fprintf(out, "%s:\n", i.res);
	}
	else {
